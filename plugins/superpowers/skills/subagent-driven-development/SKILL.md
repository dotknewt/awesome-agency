...
**Known Copilot CLI model ids (short-id form, dated 2026-08-13 — may be
stale; verify with `copilot help` before trusting for a real dispatch):**
...
This list is for Copilot CLI's `--model`/dispatch short-id format only — it
is a maintained snapshot, not a live query, and it does not apply to
Copilot Chat in VS Code's `runSubagent`, which needs a separate
`"Model Name (Vendor)"` display string (e.g. `Claude Sonnet 5 (copilot)`)
that this list does not provide. Prefer a value from this list over
guessing when dispatching through Copilot CLI, but still treat it as
best-effort: model catalogs change, and a stale id fails exactly like a
guessed one.
