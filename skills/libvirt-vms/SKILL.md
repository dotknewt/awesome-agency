---
name: libvirt-vms
description: Use when preparing or publishing Ubuntu, Debian 13, or CachyOS QCOW2 templates, managing libvirt working VMs through the libvirt-toolkit MCP server, or continuing a project-in-VM request into provider networking and a guest-access handoff.
---
...
1. Identify the MCP connection name that represents the requested host. The
   bundled `libvirt` connection is local; remote deployments should have a
...
`guest-access` by name with this record after the provider path and endpoint are
