...
- **“SDD requires a stronger final reviewer”:** this user's model preference overrides that skill's default advice.
...
Implementation: `~/.config/opencode/plugins/subagent-dispatch.ts` and global `sdd-worker`/`sdd-reviewer` agent definitions. Tests: `node --test ~/.config/opencode/skills/subagent-model-policy/tests/dispatch.test.mjs`.
